...
Each output file serves a specific purpose in supporting WASH data visualization and analysis.

1. **Key Tables**

    - `key_actual_commitment.csv`: Contains standardized commitment names used in visualizations, e.g., "Halving," "Doubling."
    ...
    - `key_jmp_category.csv`: Stores JMP categories with standardized names such as "At Least Basic" and "Safely Managed."
    ...
    - `key_value_name.csv`: Standardizes the names of values, including descriptive terms like "Full Sanitation Access" and "Full Water Access."
    - `key_value_type.csv`: Contains types of values (e.g., "total," "annual_rate_change") to distinguish between different measurements.

2. **Data Tables**

    - `table_graph_ifs.csv`: A table specifically formatted for graph visualizations, with key data fields for WASH indicators and milestone years.
    ...
    - `table_jmp.csv`: The final JMP dataset, standardized and processed, ready for integration with other datasets and visualization.

---

These output files form the foundation for the **Data Visualization** phase, where key insights and trends in WASH data will be displayed for stakeholders. Each file is stored with standardized keys and values to ensure consistent and reliable data handling.
